...
function solveSudoku(sudokuField) {
    ...
    do {
        ...
        for (var number = 1; number <= 9; number++) { // Шукаємо де є єдине місце для встановлення числа number
            ...
            f( function(sudokuSets, i, number) {return sudokuSets.getSet(number).getSegment(i); },
               function(i, index) {
            ...
            f( function(sudokuSets, i, number) { return sudokuSets.getSet(number).getRow(i);} ,
               function(i, index)              { return {x: i, y: index};                   }  );

            f( function(sudokuSets, i, number) { return sudokuSets.getSet(number).getCol(i);} ,
               function(i, index)              { return {x: index, y: i};                   }  );
        }
